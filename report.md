...
- `reference/branch`和git中概念一直，是多个`commit`形成的链。`branch`可以看做有名字的`reference`本质没有区别，在`libgit2`中其函数内部实现也是一直的;
...
`truncate`主要实现修改文件大小的功能，单位为字节，若文件小则截断尾部多余的字节，反之则填充0。最初的简要实现是利用`mktemp`系统调用把当前文件备份到临时文件，修改临时文件满足要求后提交改到到git的commit中。上述操作会对造成磁盘读取不必要的性能损失，我们注意到libgit2中代表文件的结构`blob`可以从内存中创建，这样我们只需要把原始文件的`blob`载入到内存中，通过`memcpy`等操作即可完成对文件大小的修改并避免多余的磁盘读写操作。
